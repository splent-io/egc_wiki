...
# /workspace/egc_wiki/features/, which `splent product:resolve` fills
...
# the builder stage of docker/Dockerfile.egc_wiki.prod.
#
...
features=$(python3 -c '
import os, tomllib
with open("/workspace/egc_wiki/pyproject.toml", "rb") as f:
    data = tomllib.load(f)
...
for feature in $features; do
    ...
    if [ -n "$declared_version" ]; then
        ...
        found_path=""
        for org_dir in /workspace/egc_wiki/features/*; do
            [ -d "$org_dir" ] || continue
